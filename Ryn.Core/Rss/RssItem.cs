...
namespace Ryn.Core.Rss
{
    ...
    public partial class RssItem
    {
        ...
        public RssItem(string title, string content, Uri link, string id, DateTimeOffset pubDate)
        {
            ...
            Content = new XElement(RynRssDefaults.Description, content);
            Link = new XElement(RynRssDefaults.Link, link);
            Id = new XElement(RynRssDefaults.Guid, new XAttribute("isPermaLink", false), id);
            PubDate = new XElement(RynRssDefaults.PubDate, pubDate.ToString("r"));
        }
        ...
        {
            var title = item.Element(RynRssDefaults.Title)?.Value ?? string.Empty;
            var content = item.Element(RynRssDefaults.Content)?.Value ?? string.Empty;
            if (string.IsNullOrEmpty(content))
                content = item.Element(RynRssDefaults.Description)?.Value ?? string.Empty;
            var link = new Uri(item.Element(RynRssDefaults.Link)?.Value ?? string.Empty);
            var pubDateValue = item.Element(RynRssDefaults.PubDate)?.Value;
            var pubDate = pubDateValue == null ? DateTimeOffset.Now : DateTimeOffset.ParseExact(pubDateValue, "r", null);
            var id = item.Element(RynRssDefaults.Guid)?.Value ?? string.Empty;
        ...
        {
            var element = new XElement(RynRssDefaults.Item, Id, Link, Title, Content);
