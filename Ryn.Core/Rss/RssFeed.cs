...
namespace Ryn.Core.Rss
{
    ...
    public partial class RssFeed
    {
        ...
        {
            Title = new XElement(RynRssDefaults.Title, title);
            Description = new XElement(RynRssDefaults.Description, description);
            Link = new XElement(RynRssDefaults.Link, link);
            LastBuildDate = new XElement(RynRssDefaults.LastBuildDate, lastBuildDate.ToString("r"));
        }
        ...
        public static async Task<RssFeed> LoadAsync(Stream stream)
        {
            try
            {
                ...
                var channel = document.Root?.Element(RynRssDefaults.Channel);

                if (channel == null)
                    return null;

                var title = channel.Element(RynRssDefaults.Title)?.Value ?? string.Empty;
                var description = channel.Element(RynRssDefaults.Description)?.Value ?? string.Empty;
                var link = new Uri(channel.Element(RynRssDefaults.Link)?.Value ?? string.Empty);
                var lastBuildDateValue = channel.Element(RynRssDefaults.LastBuildDate)?.Value;
                ...
                foreach (var item in channel.Elements(RynRssDefaults.Item))
                {
        ...
        public string GetContent()
        {
            ...
            var root = new XElement(RynRssDefaults.RSS, new XAttribute("version", "2.0"));
            var channel = new XElement(RynRssDefaults.Channel,
