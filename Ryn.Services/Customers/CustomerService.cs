...
namespace Ryn.Services.Customers
{
    /// <summary>
    /// Customer service
    /// </summary>
    public partial class CustomerService : ICustomerService
    {
        ...
        private readonly IRynDataProvider _dataProvider;
                ...
                if (!string.IsNullOrWhiteSpace(firstName))
                {
                    query = query
                        ...
                        .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                    z.Attribute.Key == RynCustomerDefaults.FirstNameAttribute &&
                ...
                if (!string.IsNullOrWhiteSpace(lastName))
                {
                    query = query
                        .Join(_gaRepository.Table, x => x.Id, y => y.EntityId,
                            (x, y) => new { Customer = x, Attribute = y })
                        .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                    z.Attribute.Key == RynCustomerDefaults.LastNameAttribute &&
                ...
                if (dayOfBirth > 0 && monthOfBirth > 0)
                {
                    ...
                    query = query
                        .Join(_gaRepository.Table, x => x.Id, y => y.EntityId,
                            (x, y) => new { Customer = x, Attribute = y })
                        .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                    z.Attribute.Key == RynCustomerDefaults.DateOfBirthAttribute &&
                ...
                if (!string.IsNullOrWhiteSpace(company))
                {
                    query = query
                        .Join(_gaRepository.Table, x => x.Id, y => y.EntityId,
                            (x, y) => new { Customer = x, Attribute = y })
                        .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                    z.Attribute.Key == RynCustomerDefaults.CompanyAttribute &&
                ...
                if (!string.IsNullOrWhiteSpace(phone))
                {
                    query = query
                        .Join(_gaRepository.Table, x => x.Id, y => y.EntityId,
                            (x, y) => new { Customer = x, Attribute = y })
                        .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                    z.Attribute.Key == RynCustomerDefaults.PhoneAttribute &&
                ...
                if (!string.IsNullOrWhiteSpace(zipPostalCode))
                {
                    query = query
                        .Join(_gaRepository.Table, x => x.Id, y => y.EntityId,
                            (x, y) => new { Customer = x, Attribute = y })
                        .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                    z.Attribute.Key == RynCustomerDefaults.ZipPostalCodeAttribute &&
        ...
        public virtual async Task DeleteCustomerAsync(Customer customer)
        {
            ...
            if (customer.IsSystemAccount)
                throw new RynException($"System customer account ({customer.SystemName}) could not be deleted");
        ...
        public virtual async Task<Customer> GetCustomerByIdAsync(int customerId)
        {
            return await _customerRepository.GetByIdAsync(customerId,
                cache => cache.PrepareKeyForShortTermCache(RynEntityCacheDefaults<Customer>.ByIdCacheKey, customerId));
        }
        ...
        public virtual async Task<Customer> GetCustomerByGuidAsync(Guid customerGuid)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForShortTermCache(RynCustomerServicesDefaults.CustomerByGuidCacheKey, customerGuid);
        ...
        public virtual async Task<Customer> GetCustomerBySystemNameAsync(string systemName)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(RynCustomerServicesDefaults.CustomerBySystemNameCacheKey, systemName);
        ...
        {
            var backgroundTaskUser = await GetCustomerBySystemNameAsync(RynCustomerDefaults.BackgroundTaskCustomerName);
            ...
            if (backgroundTaskUser is null)
            {
                ...
                backgroundTaskUser = new Customer
                {
                    ...
                    SystemName = RynCustomerDefaults.BackgroundTaskCustomerName,
                ...
                var guestRole = await GetCustomerRoleBySystemNameAsync(RynCustomerDefaults.GuestsRoleName);

                if (guestRole is null)
                    throw new RynException("'Guests' role could not be loaded");
        ...
        {
            var searchEngineUser = await GetCustomerBySystemNameAsync(RynCustomerDefaults.SearchEngineCustomerName);
            ...
            if (searchEngineUser is null)
            {
                ...
                searchEngineUser = new Customer
                {
                    ...
                    Active = true,
                    IsSystemAccount = true,
                    SystemName = RynCustomerDefaults.SearchEngineCustomerName,
                    CreatedOnUtc = DateTime.UtcNow,
                    LastActivityDateUtc = DateTime.UtcNow,
                    RegisteredInStoreId = store.Id
                };
            ...
            bool clearPaymentMethod = true)
        {
            ...
            if (clearCouponCodes)
            {
                await _genericAttributeService.SaveAttributeAsync<string>(customer, RynCustomerDefaults.DiscountCouponCodeAttribute, null);
                await _genericAttributeService.SaveAttributeAsync<string>(customer, RynCustomerDefaults.GiftCardCouponCodesAttribute, null);
            }

            //clear checkout attributes
            if (clearCheckoutAttributes)
                await _genericAttributeService.SaveAttributeAsync<string>(customer, RynCustomerDefaults.CheckoutAttributes, null, storeId);

            //clear reward points flag
            if (clearRewardPoints)
                await _genericAttributeService.SaveAttributeAsync(customer, RynCustomerDefaults.UseRewardPointsDuringCheckoutAttribute, false, storeId);

            //clear selected shipping method
            if (clearShippingMethod)
            {
                await _genericAttributeService.SaveAttributeAsync<ShippingOption>(customer, RynCustomerDefaults.SelectedShippingOptionAttribute, null, storeId);
                await _genericAttributeService.SaveAttributeAsync<ShippingOption>(customer, RynCustomerDefaults.OfferedShippingOptionsAttribute, null, storeId);
                await _genericAttributeService.SaveAttributeAsync<PickupPoint>(customer, RynCustomerDefaults.SelectedPickupPointAttribute, null, storeId);
            }

            //clear selected payment method
            if (clearPaymentMethod)
                await _genericAttributeService.SaveAttributeAsync<string>(customer, RynCustomerDefaults.SelectedPaymentMethodAttribute, null, storeId);
        ...
        public virtual async Task<string> GetCustomerFullNameAsync(Customer customer)
        {
            ...
            var firstName = await _genericAttributeService.GetAttributeAsync<string>(customer, RynCustomerDefaults.FirstNameAttribute);
            var lastName = await _genericAttributeService.GetAttributeAsync<string>(customer, RynCustomerDefaults.LastNameAttribute);
        ...
        public virtual async Task<string> FormatUsernameAsync(Customer customer, bool stripTooLong = false, int maxLength = 0)
        {
            ...
            switch (_customerSettings.CustomerNameFormat)
            {
                ...
                case CustomerNameFormat.ShowFirstName:
                    result = await _genericAttributeService.GetAttributeAsync<string>(customer, RynCustomerDefaults.FirstNameAttribute);
                    break;
                default:
                    break;
            }
        ...
        public virtual async Task<string[]> ParseAppliedDiscountCouponCodesAsync(Customer customer)
        {
            ...
            var existingCouponCodes = await _genericAttributeService.GetAttributeAsync<string>(customer, RynCustomerDefaults.DiscountCouponCodeAttribute);
        ...
        public virtual async Task ApplyDiscountCouponCodeAsync(Customer customer, string couponCode)
        {
            ...
            //apply new value
            await _genericAttributeService.SaveAttributeAsync(customer, RynCustomerDefaults.DiscountCouponCodeAttribute, result);
        }
        ...
        public virtual async Task<string[]> ParseAppliedGiftCardCouponCodesAsync(Customer customer)
        {
            ...
            var existingCouponCodes = await _genericAttributeService.GetAttributeAsync<string>(customer, RynCustomerDefaults.GiftCardCouponCodesAttribute);
        ...
        public virtual async Task ApplyGiftCardCouponCodeAsync(Customer customer, string couponCode)
        {
            ...
            //apply new value
            await _genericAttributeService.SaveAttributeAsync(customer, RynCustomerDefaults.GiftCardCouponCodesAttribute, result);
        }
        ...
        public virtual async Task DeleteCustomerRoleAsync(CustomerRole customerRole)
        {
            ...
            if (customerRole.IsSystemRole)
                throw new RynException("System role could not be deleted");
        ...
        public virtual async Task<CustomerRole> GetCustomerRoleBySystemNameAsync(string systemName)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(RynCustomerServicesDefaults.CustomerRolesBySystemNameCacheKey, systemName);
        ...
        public virtual async Task<int[]> GetCustomerRoleIdsAsync(Customer customer, bool showHidden = false)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForShortTermCache(RynCustomerServicesDefaults.CustomerRoleIdsCacheKey, customer, showHidden);
            ...
            return await _customerRoleRepository.GetAllAsync(query =>
            {
                return from cr in query
                       ...
                       where crm.CustomerId == customer.Id &&
                             (showHidden || cr.Active)
                       select cr;
            }, cache => cache.PrepareKeyForShortTermCache(RynCustomerServicesDefaults.CustomerRolesCacheKey, customer, showHidden));
        ...
        {
            var key = _staticCacheManager.PrepareKeyForDefaultCache(RynCustomerServicesDefaults.CustomerRolesAllCacheKey, showHidden);
        ...
        {
            return await IsInCustomerRoleAsync(customer, RynCustomerDefaults.AdministratorsRoleName, onlyActiveCustomerRoles);
        }
        ...
        {
            return await IsInCustomerRoleAsync(customer, RynCustomerDefaults.ForumModeratorsRoleName, onlyActiveCustomerRoles);
        }
        ...
        {
            return await IsInCustomerRoleAsync(customer, RynCustomerDefaults.RegisteredRoleName, onlyActiveCustomerRoles);
        }
        ...
        {
            return await IsInCustomerRoleAsync(customer, RynCustomerDefaults.GuestsRoleName, onlyActiveCustomerRoles);
        }
        ...
        {
            return await IsInCustomerRoleAsync(customer, RynCustomerDefaults.VendorsRoleName, onlyActiveCustomerRoles);
        }
        ...
        public virtual async Task<bool> IsPasswordRecoveryTokenValidAsync(Customer customer, string token)
        {
            ...
            var cPrt = await _genericAttributeService.GetAttributeAsync<string>(customer, RynCustomerDefaults.PasswordRecoveryTokenAttribute);
        ...
        public virtual async Task<bool> IsPasswordRecoveryLinkExpiredAsync(Customer customer)
        {
            ...
            var generatedDate = await _genericAttributeService.GetAttributeAsync<DateTime?>(customer, RynCustomerDefaults.PasswordRecoveryTokenDateGeneratedAttribute);
        ...
        public virtual async Task<bool> IsPasswordExpiredAsync(Customer customer)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForShortTermCache(RynCustomerServicesDefaults.CustomerPasswordLifetimeCacheKey, customer);
        ...
        public virtual async Task<IList<Address>> GetAddressesByCustomerIdAsync(int customerId)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForShortTermCache(RynCustomerServicesDefaults.CustomerAddressesCacheKey, customerId);
        ...
        public virtual async Task<Address> GetCustomerAddressAsync(int customerId, int addressId)
        {
            ...
            var key = _staticCacheManager.PrepareKeyForShortTermCache(RynCustomerServicesDefaults.CustomerAddressCacheKey, customerId, addressId);
