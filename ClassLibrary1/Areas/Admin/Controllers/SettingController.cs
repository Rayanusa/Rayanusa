...
using Ryn.Core.Domain.Security;
...
using Ryn.Services.Media.RoxyFileman;
...
namespace Ryn.Web.Areas.Admin.Controllers
{
    public partial class SettingController : BaseAdminController
    {
        ...
        private readonly IRynFileProvider _fileProvider;
        ...
        public virtual async Task<IActionResult> ChangeStoreScopeConfiguration(int storeid, string returnUrl = "")
        {
            ...
            if (store != null || storeid == 0)
            {
                await _genericAttributeService
                    .SaveAttributeAsync(await _workContext.GetCurrentCustomerAsync(), RynCustomerDefaults.AdminAreaStoreScopeConfigurationAttribute, storeid);
            }
        ...
        public virtual async Task<IActionResult> ChangeEncryptionKey(GeneralCommonSettingsModel model)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(newEncryptionPrivateKey) || newEncryptionPrivateKey.Length != 16)
                    throw new RynException(await _localizationService.GetResourceAsync("Admin.Configuration.Settings.GeneralCommon.EncryptionKey.TooShort"));

                var oldEncryptionPrivateKey = securitySettings.EncryptionKey;
                if (oldEncryptionPrivateKey == newEncryptionPrivateKey)
                    throw new RynException(await _localizationService.GetResourceAsync("Admin.Configuration.Settings.GeneralCommon.EncryptionKey.TheSame"));
            ...
            try
            {
                ...
                switch (_fileProvider.GetFileExtension(iconsFile.FileName))
                {
                    case ".ico":
                        ...
                        commonSettings.FaviconAndAppIconsHeadCode = string.Format(RynCommonDefaults.SingleFaviconHeadLink, storeScope, iconsFile.FileName);
                    ...
                    case ".zip":
                        ...
                        var headCodePath = _fileProvider.GetAbsolutePath(string.Format(RynCommonDefaults.FaviconAndAppIconsPath, storeScope), RynCommonDefaults.HeadCodeFileName);
                        if (!_fileProvider.FileExists(headCodePath))
                            throw new Exception(string.Format(await _localizationService.GetResourceAsync("Admin.Configuration.Settings.GeneralCommon.FaviconAndAppIcons.MissingFile"), RynCommonDefaults.HeadCodeFileName));
                ...
                var oldFaviconIconPath = _fileProvider.GetAbsolutePath(string.Format(RynCommonDefaults.OldFaviconIconName, storeScope));
